...
public class AStar {
	public void astar(Container container, String args){
		...
		waitList //the list of nodes to be evaluated
		visited //the list of nodes already evaluated
		add the start node to waitList
		
		loop
		    currentVertices = node in waitList with the lowest f(n)
		    remove currentVertices from waitList
		    add currentVertices to visited
		 ...
		 * 
		 * The time complexity of worst case is O(N * N)
		 * The worst case is all the nodes go into waitList, and traverse all their children nodes.
		 * 
		 * 
		 */
		//System.out.println("============================");//--
...
//		recordPath.add(start);
		...
		while(currentVertices != endNum){
			...
			for(int i = 0; i < 20 ; i++){
				...
				int truePre = currentVertices;//pre in graph,not in traverse sequence
				int children = i;
				
				//=============
				List<Integer> tails = new ArrayList<Integer>();//true pre in which row of recordPath
...
//=====================================	
	...
	public ArrayList<Integer> copyWhichLine(ArrayList<ArrayList<Integer>> recordPath, int truePre){
		ArrayList<Integer> result = new ArrayList<Integer>();//copy from which line? or which lines?
		for(int i = 0; i < recordPath.size(); i++){
			int secondLast = recordPath.get(i).size() - 2;
			if(recordPath.get(i).get(secondLast) == truePre){
				result.add(i);
